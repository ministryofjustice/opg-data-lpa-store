...
func TestGetListWithImages(t *testing.T) {
	...
	for scenario, tc := range testcases {
		t.Run(scenario, func(t *testing.T) {
			...
			var getJSON struct {
				Lpas []map[string]json.RawMessage `json:"lpas"`
			}
			json.NewDecoder(getResp.Body).Decode(&getJSON)

			assert.Len(t, getJSON.Lpas, 3)
			assert.Contains(t, uids, extract[string](getJSON.Lpas[0]["uid"]))
			assert.Contains(t, uids, extract[string](getJSON.Lpas[1]["uid"]))
			assert.Contains(t, uids, extract[string](getJSON.Lpas[2]["uid"]))

			for _, lpa := range getJSON.Lpas {
				restrictionsAndConditionsImages := extract[[]map[string]string](lpa["restrictionsAndConditionsImages"])

				assert.Regexp(t, tc.pathRe(extract[string](lpa["uid"])), restrictionsAndConditionsImages[0]["path"])
			}
		})
	}
}
...
func extract[T any](msg json.RawMessage) T {
	var val T
