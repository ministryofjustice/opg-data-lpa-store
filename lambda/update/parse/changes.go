	...
	for i, change := range p.changes {
		if change.Key == key {
			if !bytes.Equal(change.Old, []byte("null")) {
				p.errors = append(p.errors, shared.FieldError{Source: change.Source("/old"), Detail: "must not be provided"})
			}

			if err := json.Unmarshal(change.New, v); err != nil {
				p.errors = append(p.errors, shared.FieldError{Source: change.Source("/new"), Detail: "wrong type"})
