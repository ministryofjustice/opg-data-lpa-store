...
func (p changeWithPosition) Source() string {
	return fmt.Sprintf("/changes/%d", p.pos)
}

func (p changeWithPosition) SourceFormat(after string) string {
...
// OutOfRange can be used with [Each] when the index is not in an expected range. It adds an out of range error for all changes.
func (p *Parser) OutOfRange() []shared.FieldError {
	for _, change := range p.changes {
		p.errors = append(p.errors, shared.FieldError{Source: change.SourceFormat("/key"), Detail: "index out of range"})
	}
...
func (p *Parser) Field(key string, v any) *Parser {
	for i, change := range p.changes {
		if change.Key == key {
			if !bytes.Equal(change.Old, []byte("null")) {
				p.errors = append(p.errors, shared.FieldError{Source: change.SourceFormat("/old"), Detail: "must not be provided"})
			}

			if err := json.Unmarshal(change.New, v); err != nil {
				p.errors = append(p.errors, shared.FieldError{Source: change.SourceFormat("/new"), Detail: "wrong type"})
			}
...
// OptionalField will JSON unmarshal the specified key into v. Ignoring the key if it does not exist.
func (p *Parser) OptionalField(key string, v any) *Parser {
	for i, change := range p.changes {
		if change.Key == key {
			if !bytes.Equal(change.Old, []byte("null")) {
				p.errors = append(p.errors, shared.FieldError{Source: change.SourceFormat("/old"), Detail: "must not be provided"})
...
// ValidatedField will JSON unmarshal the specified key into v, then run fn
// which may return any errors to add to the Parser.
func (p *Parser) ValidatedField(key string, v any, fn func() []shared.FieldError) *Parser {
	for i, change := range p.changes {
		if change.Key == key {
			if !bytes.Equal(change.Old, []byte("null")) {
				p.errors = append(p.errors, shared.FieldError{Source: change.SourceFormat("/old"), Detail: "must not be provided"})
			...
			if err := json.Unmarshal(change.New, v); err != nil {
				...
				for _, error := range fn() {
					p.errors = append(p.errors, shared.FieldError{Source: change.SourceFormat("/new"), Detail: error.Detail})
				}
			}
...
func (p *Parser) Each(fn func(int, *Parser) []shared.FieldError, required ...int) *Parser {
	...
	for _, change := range p.changes {
		...
		if len(parts) != 3 || parts[0] != "" {
			p.errors = append(p.errors, shared.FieldError{Source: change.SourceFormat("/key"), Detail: "require index"})
			continue
		}
...
//			Consumed()
//	})
func (p *Parser) Prefix(prefix string, fn func(*Parser) []shared.FieldError) *Parser {
...
// OptionalPrefix will run fn with a [Parser] of any changes with the specified prefix.
func (p *Parser) OptionalPrefix(prefix string, fn func(*Parser) []shared.FieldError) *Parser {
	var matching, remaining []changeWithPosition
