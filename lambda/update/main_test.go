...
func TestHandleEvent(t *testing.T) {
	store := &mockStore{get: shared.Lpa{Uid: "1", LpaInit: shared.LpaInit{Donor: shared.Donor{Person: shared.Person{FirstNames: "Johm"}}}}}
	l := Lambda{
		store:    store,
	...
	assert.Contains(t, resp.Body, `"John"`)
	assert.Equal(t, shared.Lpa{
		Uid:     "1",
		LpaInit: shared.LpaInit{Donor: shared.Donor{Person: shared.Person{FirstNames: "John"}}},
	}, store.put)
}

func TestHandleEventWhenOldDoesNotMatch(t *testing.T) {
	l := Lambda{
	...
	assert.JSONEq(t, `{"code":"INVALID_REQUEST","detail":"Invalid request","errors":[{"source":"/changes/0/old","detail":"does not match existing value"}]}`, resp.Body)
}

func TestHandleEventWhenIncorrectPointer(t *testing.T) {
	l := Lambda{
		store:    &mockStore{get: shared.Lpa{Uid: "1"}},
		verifier: &mockVerifier{ok: true},
		logger:   logging.New(io.Discard, ""),
	...
	resp, err := l.HandleEvent(context.Background(), events.APIGatewayProxyRequest{
		Body: `{"type":"SCANNING_CORRECTION","changes":[{"key":"/donor/whatEven","old":"Johm","new":"John"}]}`,
	})
	assert.Nil(t, err)
