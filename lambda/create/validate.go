...
func Validate(lpa shared.LpaInit) []shared.FieldError {
	...
	return flatten(
		...
		validateIf(lpa.CertificateProvider.CarryOutBy == shared.CarryOutByOnline,
			required("/certificateProvider/email", lpa.CertificateProvider.Email)),
		validateAttorneys("/attorneys", lpa.Attorneys),
		validateIfElse(activeAttorneyCount == 0,
		...
		validateIf(lpa.HowAttorneysMakeDecisions == shared.HowMakeDecisionsJointlyForSomeSeverallyForOthers,
		...
		validateIf(lpa.HowReplacementAttorneysStepIn == shared.HowStepInAnotherWay,
