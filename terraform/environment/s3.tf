...
module "s3_replica_kms_key" {
  ...
  bucket_arns = [
    module.s3_data_store_eu_west_1.bucket.arn,
    module.s3_data_store_eu_west_2.bucket.arn,
  ]
...
module "s3_lpa_store_static_eu_west_1" {
  ...
  s3_access_logging_bucket_prefix = "s3-access-logs-opg-lpa-store-${local.environment.account_name}-eu-west-1"
  s3_replication_role             = aws_iam_role.s3_replication_role
  providers = {
    aws = aws
  }
...
module "s3_lpa_store_static_eu_west_2" {
  ...
  s3_access_logging_bucket_prefix = "s3-access-logs-opg-lpa-store-${local.environment.account_name}-eu-west-2"
  s3_replication_role             = aws_iam_role.s3_replication_role
  providers = {
    aws = aws.eu-west-2
  }
